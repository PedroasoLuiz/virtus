...
export async function listar({
  ...
  const { itens, total } = await service.listarRecebimentos(empresaId, filtro, {
    page,
    perPage,
  });
...
export async function obter({
  ...
  const empresaId = empresaObrigatoria(ctx);
  return ok(
    recebimentoSchema.parse(
      await service.obterRecebimento(empresaId, params.id),
    ),
  );
}
...
export async function parcelasEmAberto({
  ...
  const empresaId = empresaObrigatoria(ctx);
  return ok(
    carteiraSchema.parse(
      await service.carteiraDoCliente(empresaId, query.clienteId),
    ),
  );
}
...
export async function criar({
  body,
  ctx,
}: Entrada<CriarRecebimentoBody, undefined, unknown>) {
  const empresaId = empresaObrigatoria(ctx);

  const recebimento = await service.registrarRecebimento(
    empresaId,
    ctx.usuarioId,
    {
